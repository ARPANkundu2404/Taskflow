...
export default function Dashboard() {
  ...
  const fetchTickets = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/api/tickets/user/${encodeURIComponent(
          user.email
        )}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok && (data.status === 200 || data.status === 201)) {
        ...
        const msg =
          data?.message || `Failed to create ticket (HTTP ${res.status})`;
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8080/api/tickets/details`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleStatusUpdate = async (ticketId, newStatus) => {
    ...
    try {
      const res = await fetch(`http://localhost:8080/api/tickets/status`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm space-x-3">
                      ...
                      {user?.role === "ROLE_ADMIN" && (
                        <button
                          onClick={() =>
                            handleDeleteClick(ticket.id, ticket.subject)
                          }
